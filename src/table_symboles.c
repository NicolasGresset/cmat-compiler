...
void stack_id_push(struct table_hachage_t * hash_tab)
{
    ...
    {
        fprintf(stderr, "Erreur lors de la création d'une nouvelle pile d'identificateurs\n");
        exit(1);
    }
    ...
    if (noeud == NULL)
    {
        fprintf(stderr, "Erreur lors de la création d'un noeud de la table de hachage\n");
        exit(1);
    }
                                 ...
                                 const char * name)
{
    if (hash_tab->alveoles == NULL)
    {
        fprintf(stderr, "Recherche impossible : Erreur pas de pile existante\n");
    }
    ...
    while(noeud != NULL && noeud->stack_id != NULL)
    {
        ...
        {
            if (strncmp(name, stack_id->items[i]->name, TAILLE_MAX_TOKEN) == 0)
            {
...
{
    printf("-----Pile du Contexte Courrant-----\n");
    if (hash_tab == NULL)
    {
        printf("Stack vide\n");
    }
