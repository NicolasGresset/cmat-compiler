 ...
 */
void save_float_as_int_symbol(struct symbol *symbol,
  ...
  case NAME:
    ...
    fprintf(code->out, "  mfc1 %s, %s\n", registers[register_number_int],
            registers[register_number_float]); // move from coprocessor 1 to $register_number_int
    fprintf(code->out, "  sw %s, %s\n", registers[register_number_int],
            symbol->u.id.name);
    break;
  default:
 ...
 * @param register_number_int
 */
void save_int_as_float_symbol(struct symbol *symbol,
                                struct assembly_code *code,
                                int register_number_float,
                                int register_number_int) {
...
void manage_copy(struct quad *quad, struct assembly_code *code) {
  ...
  if (quad->sym1->u.id.type == ENTIER) {
    ...
    } else if (is_symbol_float(quad->sym2)) { // sym1 est un entier et sym2 un
                                              // flottant
    ...
    } else if (is_symbol_int(quad->sym2)) { // sym1 est un flottant et sym2 un
                                            // entier
